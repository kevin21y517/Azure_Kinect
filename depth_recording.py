...
class Recording():
    ...
    def get_user_input(self):
        ...
        cv2.resizeWindow(window_name, 640, 576)
    ...
    def rename_video_file(self):
        # 构建最终文件名
        final_filename = f"{self.ID}_output_{self.elapsed_time}s_{self.frame_rate:.2f}fps.mkv"
        final_video_path = os.path.join(self.output_folder, final_filename)

        # 重命名文件
        os.rename(self.temp_video_filename, final_video_path)
        print(f"Video successfully renamed to {final_filename}")
    ...
    def main(self):
        ...
        temp_filename = "temp_output.mkv"
        self.temp_video_filename = os.path.join(self.output_folder, temp_filename)
        # video_filename = os.path.join(self.output_folder, f"{self.ID}_output_{self.elapsed_time}s_{self.frame_rate}fps.mkv")
        device = pykinect.start_device(config=device_config, record=True, record_filepath=self.temp_video_filename)
        ...
        while True:
            ...
            if cv2.waitKey(1) == 32:
                ...
                self.get_user_input()
                device.close()
                self.rename_video_file()  # 重命名视频文件
                break
            if cv2.waitKey(1) == 27:
                self.should_exit = True
                device.close()
